...
2. **Parse the Article Bundle** — read frontmatter using the fixed contract, read Markdown body, extract every `<script type="application/ld+json">` block, load any sidecar JSON-LD files, and remove extracted script blocks from the Markdown that will be sent to `Article.content`.
  - **Sanitize the body before upload** — remove any `# ` (H1) heading line from the body before saving `Article.content`, and reject unresolved screenshot placeholder lines matching `> [截图占位符：` or `> [Screenshot Placeholder:` with file/line references. See `references/publish-workflow.md` §Body Sanitization.
