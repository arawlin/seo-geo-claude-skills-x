...
argument-hint: "<topic> [articles=\"6\"] [language=\"en\"] [output_dir=\"...\"]"
metadata:
  author: aaron-he-zhu
  version: "9.2.0"
  geo-relevance: "high"
...
```text
Run the full series workflow for [topic] into [output_dir]
```
...
- **Reads**: topic, audience, language, output directory, optional competitor
  or site inputs, plus stage artifacts as they are generated.
...
- **Output Directory**: final destination path

## Instructions

1. **Validate top-level input**
   - Confirm `topic`, `output_dir`, `language`, and article count.
   - Stop with `NEEDS_INPUT` if `topic` or `output_dir` is missing.
...
3. **Run the batch generator**
   - Pass through the `00-series-plan.json` path from the planner.
...
5. **Return one orchestration summary**
   - Report what completed, where files were written, and which article files
...
- [ ] Final summary lists the delivery directory and unresolved concerns
