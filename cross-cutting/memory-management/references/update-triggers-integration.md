...
### After Ranking Check
```markdown
1. Update memory/hot-cache.md -> Hero Keywords table (current ranks)
...
4. Update "Last Metrics Update" date in memory/hot-cache.md
5. If hero keyword moves +/-5 positions, create alert note
```

### After Competitor Analysis
```markdown
1. Update memory/hot-cache.md -> Primary Competitors section (DA, position vs. them)
...
2. Extract top 3-5 action items -> memory/hot-cache.md Current Optimization Priorities
...
2. Update all metrics in memory/hot-cache.md Key Metrics Snapshot
...
```markdown
1. Review memory/hot-cache.md for items not referenced in 30 days
2. Move stale items to appropriate cold storage
3. Create monthly snapshot: memory/monitoring/snapshots/YYYY-MM-hot-cache-snapshot.md
...
- Promote top opportunities to memory/hot-cache.md if high-value
- Update glossary if new terminology emerges

### When rank-tracker runs:
- Update memory/monitoring/rank-history/
- Refresh memory/hot-cache.md Hero Keywords table
...
- Refresh memory/hot-cache.md Primary Competitors section
...
- Promote gap opportunities to memory/hot-cache.md priorities
...
- Update memory/hot-cache.md Key Metrics with latest score
...
- Update CITE Score in memory/hot-cache.md Key Metrics Snapshot
